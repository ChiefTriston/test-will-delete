...
# Initialize spaCy pipeline once
nlp_spacy = spacy.load("en_core_web_sm")
nlp_spacy.add_pipe("negex")

# Speaker embedding model loaded lazily in run
def run(context):
    """
    Process Tier 1 tags and transcripts to produce Tier 2 emotion tags.
    Skips stage if Stanza resources are unavailable to avoid network fetch errors.
    """
    # Attempt to load Stanza from local cache only
    try:
        import stanza
        from stanza.resources.common import DownloadMethod
        print("[Tier2] Loading Stanza from local cache...")
        nlp_stanza = stanza.Pipeline(
            lang='en', processors='tokenize,mwt,pos',
            download_method=DownloadMethod.REUSE_RESOURCES
        )
        print("[Tier2] Stanza ready; proceeding with tagging.")
    except Exception as e:
        print(f"[Tier2] Stanza unavailable ({e}); skipping Tier2 stage.")
        return
    ...
    # Load speaker embedding model once (local load, no HF)
    from modules.models.spk_embedder import load_embedder
    ...
    for spk in context['speaker_ids']:
        ...
        tier1 = safe_load_json(os.path.join(spk_dir, 'tier1_tags.json'))
        transcript = safe_load_json(os.path.join(spk_dir, 'transcript.json'))
        prosody = safe_load_json(os.path.join(spk_dir, 'prosody_trend.json'))
        drift = safe_load_json(os.path.join(spk_dir, 'drift_vector.json'))
