...
from bidict import bidict, Bidict
...
class ReIDCallback(ABC):
    ...
    def on_match(self, orig_label: int, speaker_id: str, score: float, avg_emb: np.ndarray) -> None:
        pass

    def on_new_speaker(self, orig_label: int, new_id: str, avg_emb: np.ndarray) -> None:
        pass
...
class PairCollector(ReIDCallback):
    ...
    def on_match(self, orig_label: int, speaker_id: str, score: float, avg_emb: np.ndarray) -> None:
        prototype = ReIDMemory.prototypes.get(speaker_id, avg_emb)
        self.positives.append((prototype, avg_emb))

    def on_new_speaker(self, orig_label: int, new_id: str, avg_emb: np.ndarray) -> None:
        if ReIDMemory.prototypes:
            random_proto = choice(list(ReIDMemory.prototypes.values()))
    ...
    def cluster(self, sim_matrix: np.ndarray, features: np.ndarray) -> np.ndarray:
        ...
        for i in range(sim_matrix.shape[0]):
            for j in range(i + 1, sim_matrix.shape[0]):
                if sim_matrix[i, j] > CLUSTER_MIN_SIM:
...
class ReIDMemory(nn.Module):
    prototypes: Dict[str, np.ndarray] = {}  # Instance-level for shared access
    ...
    def __init__(
        ...
        self.label_map: Bidict[int, str] = bidict()
    ...
    def _learned_similarity(self, a: np.ndarray, b: np.ndarray) -> float:
        ...
        b_t = torch.from_numpy(b).float().to(self.device)
        return (a_t @ self.W @ b_t).item()

    def _contrastive_refine(self, emb: np.ndarray) -> np.ndarray:
        emb_t = torch.from_numpy(emb).float().to(self.device).unsqueeze(0)
        refined = self.contrastive_head(emb_t).squeeze(0).cpu().numpy()
        return refined / np.linalg.norm(refined + 1e-6)
    ...
    def re_id(
        ...
        assert embs.shape[1] == EMB_DIM, f"Expected embs dim {EMB_DIM}, got {embs.shape[1]}"
        assert len(labels) == embs.shape[0], "Labels and embs mismatch"
        if slices:
            assert len(slices) == embs.shape[0], "Slices and embs mismatch"

        labels = np.asarray(labels, dtype=int)
        embs_t = torch.from_numpy(embs).float().to(self.device).unsqueeze(0)
        assert embs_t.device == self.device, "Device mismatch"
    ...
    def _filter_anomalies(self, embs: np.ndarray) -> np.ndarray:
        ...
        detector.fit(embs)
        inliers = detector.predict(embs) == 1
        return embs[inliers]
...
def time_aware_sim(embs, slices, temporal_weight=0.5, decay=TEMPORAL_DECAY):
    # 1) cosine sim
    sim = cosine_similarity(embs)
    # 2) temporal decay
    mids = np.array([(s+e)/2 for s,e,_ in slices])
    gaps = np.abs(mids[:,None] - mids[None,:])
    probs = np.array([p for _,_,p in slices])
    mod = (probs[:,None] + probs[None,:])/2
    temporal = mod * np.exp(-gaps/decay)
    np.fill_diagonal(temporal, 1.0)
    # 3) combine & normalize
    comb = (1-temporal_weight)*sim + temporal_weight*temporal
    mn, mx = comb.min(), comb.max()
    return (comb - mn)/(mx - mn + 1e-6)

def temporal_cluster(sim, slices, min_sim=CLUSTER_MIN_SIM, features=None):
    if features is None:
        raise ValueError("Must pass `features` (slice embeddings) to temporal_cluster")
    return ReIDMemory().clusterer.cluster(sim, features)
    min_sim: float = CLUSTER_MIN_SIM,
    features: np.ndarray = None
) -> np.ndarray:
    if features is None:
        features = np.zeros((sim.shape[0], EMB_DIM))  # Placeholder; pass actual embs from caller
    return ReIDMemory().clusterer.cluster(sim, features)