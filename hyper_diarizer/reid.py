...
def detect_overlaps(audio, slices, labels, embs):
    ...
    """
    try:
        # 1) Compute per-speaker energy for thresholding
        energies = {}
        for label in np.unique(labels):
            segs = [
                audio[int(s * SAMPLE_RATE):int(e * SAMPLE_RATE)]
                for idx, (s, e, _) in enumerate(slices)
                if labels[idx] == label
            ]
            energies[label] = (
                np.mean([np.mean(seg ** 2) for seg in segs]) if segs else 0
            )
        ...
        # 2) Intra-slice detection via spectral features + classifier
        for i, (s, e, _) in enumerate(slices):
            ...
            # If either feature is high, run the classifier
            ...
            if i < len(slices) - 1:
                ...
                norm_energy = gap_energy / max(
                    energies.get(labels[i], 0),
                    energies.get(labels[i + 1], 0),
                    1e-6
                )
